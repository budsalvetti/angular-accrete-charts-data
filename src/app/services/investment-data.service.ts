...
export class InvestmentDataService {
  ...
  public readonly mockCategorySeedAmounts = {
    CAPEX: 15000000,
    Revenue: 100000,
  ...
  // utility to get random numbers for mock industry scores
  ...
  public getIndustryData(investmentCategory?: string): Observable<any[]> {
    ...
    for (let industry of this.industries) {
      ...
      let yearTotal = investmentCategory
        ? this.mockCategorySeedAmounts[investmentCategory]
        : 0;
      ...
      for (let year of this.years) {
        ...
        yearObj['total'] = yearTotal;
        yearObj['score'] = this.roundUp(Math.random(), 1);
  ...
  /**
   * getIndustryScoreChartData
   * @returns Observable<any[]>
   * @description creates an array of chart data suitable to plugin in to canvasJS chart
   * by adapting the raw industry data as such
   */
  public getIndustryScoreChartData(): Observable<any[]> {
  ...
  public getChartData(entity: string, investmentCategory): Observable<any[]> {
    ...
    } else if (entity === 'INDUSTRY_SCORE') {
      return this.getIndustryScoreChartData();
    }
  }
}
